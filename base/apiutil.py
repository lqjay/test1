...
class RequestBase:
    ...
    def extract_data(self, testcase_extarct, response):
        ...
        """
        try:
            logs.info(f'开始提取数据，完整配置: {testcase_extarct}')
            logs.info(f'接口响应内容: {response[:200]}...')  # 打印前200个字符
            ...
            for key, value in testcase_extarct.items():
                ...
                if '$' in value:
                    ...
                    if ext_json:
                        ...
                        logs.info('提取接口的返回值：%s', str(extract_data))
                    else:
...
if __name__ == '__main__':
    ...
    case_info = get_testcase_yaml('../base/test1.yaml')
    # print(case_info)
    req = RequestBase()
